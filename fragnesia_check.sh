...
# =============================================================================
# CHECK 1 — Kernel version / build date (FIXED)
# =============================================================================
check_kernel_version() {
    ...
    # Improved date extraction - handles Ubuntu-style builds missing year
    local parsed_date
    parsed_date=$(echo "$kbuild" | grep -oP '(Mon|Tue|Wed|Thu|Fri|Sat|Sun)\s+\w+\s+\d{1,2}\s+[\d:]+\s+\w+' | tail -1)

    if [[ -n "$parsed_date" ]]; then
        # Try parsing with current year first
        local current_year=$(date +%Y)
        local try_date="${parsed_date} ${current_year}"
        
        kbuild_epoch=$(date -d "$try_date" +%s 2>/dev/null || echo 0)
        
        if [[ "$kbuild_epoch" -gt 0 && "$patch_epoch" -gt 0 ]]; then
            if [[ "$kbuild_epoch" -ge "$patch_epoch" ]]; then
                pass "Kernel appears compiled on/after patch date — ${parsed_date} ${current_year}"
                MITIG_FLAGS+=("kernel_build_date_ok")
            else
                fail "Kernel compiled BEFORE patch date — ${parsed_date} ${current_year}"
                VULN_FLAGS+=("kernel_build_date_before_patch")
            fi
        else
            warn "Could not determine build epoch. Manual check required."
    ...
    # Kernel series warning
    if echo "$kver" | grep -qP '^6\.(1|2|3|4|5|6|7|8)\.\d+'; then
